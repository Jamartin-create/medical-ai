...
export function getPageParams(params: any) {
    ...
    function getPageResult(list: any[], total: number) {
        const pageCount = Math.ceil(total / limit) // 计算页码
        const hasNext = offset + 1 < pageCount // 计算是否有下一页
        const hasPrevious = offset > 0 // 计算是否有上一页
        return { list, total, pageCount, hasNext, hasPrevious } // 返回前端需要的所有参数
    }
